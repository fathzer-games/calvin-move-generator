...
//		Board board = Board.from("r3k2r/p1p1qpb1/bn1ppnpB/3PN3/1p2P3/1PN2Q1p/P1P1BPPP/R3K2R b KQkq - 0 2");
		final Board board = Board.from("r3k2r/2pb1ppp/2pp1q2/1Q6/pnP1B3/1P2P3/P2N1PPP/R3K2R b KQkq - 1 2");
		Move move = Move.fromUCI("e8c8", Move.CASTLE_FLAG);
		Assertions.assertTrue(movegen.isPseudoLegal(board, move));
		move = Move.fromUCI("e8g8", Move.CASTLE_FLAG);
		Assertions.assertTrue(movegen.isPseudoLegal(board, move));
	}
	...
	public void testBenchSuite() {
		FENS.stream().map(Board::from).forEach(board -> pseudoLegalPerft(board, 5));
	}
	...
	public long pseudoLegalPerft(Board board, int depth) {
		...
		for (Move move : moves) {
			if (!movegen.isPseudoLegal(board, move)) {
				Assertions.fail(FEN.toFEN(board) + " " + Move.toUCI(move) + " at depth " + depth + " after moves "
						+ Arrays.asList(board.getMoves()));
			}
